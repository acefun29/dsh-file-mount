...
- **Token 节省统计与提醒（已实现，v0.2.0）**：
  - 估算：`src/tokens.ts` 按「字符数 ÷ 4」估算（min 1）；去重 = 整个被去重窗口；增量 = 已覆盖区间；new/remount 计 0；
  - 载体：挂载消息 source 加 `form:'notice'` + `summary` + `savedTokens`；去重决策注入一条 head-only note 消息（工具结果仍为去重 marker）；
  - 展示：对话上下文行折叠摘要（notice summary，如「saved ≈ N tokens」「+L5-6 - saved ≈ N tokens」）、挂载文件视图顶部累计汇总行、每个文件行的单文件节省数；
  - 累计语义：同 hash 并集相加；文件变化重挂载时保留历史累计；恢复重放与浏览器折叠同规则，跨会话一致；
  - 边界：只算因本插件去重而不再进入上下文的部分，不重复计算；精确 tokenizer 后续再议。

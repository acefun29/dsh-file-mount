...
7. **新鲜度（注意力衰减）**：每个挂载段记录挂载时的上下文位置（最近一轮请求的**全部输入 token——未缓存 + 缓存命中之和**（DSH 的 usage 是 DISJOINT 计数，`inputTokens` 只含未命中缓存部分）+ 注入块估算）；会话每轮请求的 usage 增量推进「当前上下文长度」，段距上下文尾部的漂移率决定新鲜度级别（U 型注意力曲线的尾部注意区=新鲜，> 阈值=过期）。**过期段自动摘除账本**（下次 read 重新发送，token 换可靠性），摘除历史保留——重新挂载时过期次数 +1，UI 显示徽标。全程无定时器：过期检查只在 read 时惰性执行。

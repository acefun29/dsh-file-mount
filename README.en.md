...
- **UI side**: the Mounted Files tab is a dashboard with per-file progress bars, a path search box, sorting by net savings or path, and a session net-savings header converted to a rough CNY figure; remounted rows carry a "changed, remounted" badge.
...
```sh
pnpm install
pnpm test        # vitest: unit + real read/write loop integration + persistence + compaction + client components
